...
contract DeployAll is Script {
    function run() external {
        ...
        deployFallbackAlways();
        deployFallbackERC20(deployer);
        deployFallbackNFT(deployer);
        vm.stopBroadcast();
    }

    function deployFallbackAlways() internal {
        FallbackAlways fallbackAlways = new FallbackAlways();
        console.log("FallbackAlways deployed at", address(fallbackAlways));
    }

    function deployFallbackERC20(address deployer) internal {
    ...
    function deployFallbackNFT(address deployer) internal {
        ...
        require(nftOperatorAllowlist != address(0), "NFT_OPERATOR_ALLOWLIST is required");
